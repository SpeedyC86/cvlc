 ...
 * @version  0.1.0
 */
Cvlc = Fn.inherits('Informer', function Cvlc(options, callback) {
...
Cvlc.setMethod(function getInstance(force, callback) {
	...
	this.hinder = Fn.hinder(function initVlc(done) {
        ...
        '-' // Start by listening to the input by default
			...
			console.log('Creating cvlc process:', args);
      ...
      that.connection.on('data', function (data) {
        if (that.debug) console.log('Telnet connection data:', data.toString('ascii'));
        that.emit('data', data.toString('ascii'))
      })
      that.connection.on('end', function (err) {
        if (that.debug) console.log('Telnet connection ended');
        that.emit('end', err)
      })
...
Cvlc.setMethod(function _queueFile(file, do_pause, callback) {
  ...
  if (that.debug) console.log('File:', file, 'do pause:', do_pause, 'callback:', callback);
...
Cvlc.setMethod(function kill() {
	...
	console.log('Killing cvlc process');
